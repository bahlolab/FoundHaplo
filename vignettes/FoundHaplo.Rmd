...
Create_hap_VCF | Takes a phased input data file with columns in VCF format and separates haplotypes into two columns
...
**NOTE: Run Generate_FH_score function on the R command line since the system() command used in the function to utilise vcfools/bcftools may not work on R studio..**
  
  ### Calculate the corresponding base pair positions and trim the dataset to contain +-10cM on either side of the disease-causing variant
  ```{r}
library(FoundHaplo)
library(vcfR)
library(data.table)
...
```{r}
# FAME1_test_cohort is a VCF file with imputed SNP genotyped samples to test using FoundHaplo
...
```{r}
if(!dir.exists(paste0(tempdir(), "/1"))){dir.create(paste0(tempdir(), "/1"))} # create a temporary directory to save disease haplotypes
...
sample_info # sample_info has three lines with instructions to pedigree phase three disease haplotypes in three separate VCF files, using the FAME1_disease_cohort
...
Analyse_FH("FH_IBD_scores.txt",temp_DIR,0.99,from_control=FALSE) 
# Samples predicted to carry the disease haplotypes will be plotted in red and will be returned as output in the function
setwd(orig_DIR)
```{r}
