...
#'
#' @description
#' Takes a VCF file with at least two known disease individuals and create disease haplotypes based on trio/duo or related data.
#' @details
#' Make sure the input_vcf chromosome column doesn't have the "chr" prefix.
#' Loading ANNOVAR files to annotate population freqencies takes most of the time and advised to load them to R and run the rest of the code in the function to create disease haplotypes.
#' @param input_vcf Path to a VCF file  (type \code{"character"})
#' @param dir_output Directory path to a directory to save the output VCF files (type \code{"character"})
#' @param sample_info_file Path to a tab delimitted .txt file with sample names and type of phasing to be used included in a new line, include sample names as in the VCF file in mentioned order.
...
#' @return Write a valid new VCF file in dir_output "h1" column contains the disease haplorype saved as homozygous genotypes.
#' @import vcfR
#' @import foreach
#' @import doParallel
#' @import parallel
...
Phasing_by_pedigree=function(input_vcf,dir_output,sample_info_file)
{
    ...
    system.time(foreach(line = 1:nrow(sample_info),.packages=c('foreach','dplyr','vcfR','FoundHaplo','stringi','stringr')) %dopar% {
        ...
        x1=rep(paste(replicate(number_of_FORMAT_fields, ":100"), collapse = ""),nrow(data_file))
        x2=data_file$h1
        data_file$h1=paste0(x2,x1)
        x1=rep(paste(replicate(number_of_FORMAT_fields, ":100"), collapse = ""),nrow(data_file))


        write.table(input_file_vcf@meta,paste0(dir_output,"/",sample_info[line,1],".vcf"),sep = "\t",quote=FALSE, row.names=FALSE,col.names = FALSE)
