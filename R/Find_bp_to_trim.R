...
#'
#' @description
#' Takes a text file with disease causing variants written in "FAME1.chr8.119379052." format, variants added in new line or a vector of variants in the same format and calculate base pair values corresponding to +-10cM from the disease variant locus
...
Find_bp_to_trim=function(input_vector,geneticMap_DIR,output_file,size_to_trim_cM=10)
{
  ...
  for(j in 1:length(input_vector))
  {
    ...
    path=paste0(geneticMap_DIR,"/genetic_map_GRCh37_",chr,".txt") # geneticMap files are in input_files/public_data/genetic_map_HapMapII_GRCh37
