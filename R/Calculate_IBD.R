...
#' \item position_cM, position in centiMorgans (type \code{"numeric"})
...
#' disease_file <-fread(paste0(temp_DIR,"/",paste(c("NA11920_NA11920","Affected_parent_NA11920_Affected_parent_NA11920","Unaffected_parent_NA11920_Unaffected_parent_NA11920"), collapse = ','),".vcf"), skip = "#CHROM")
...
#' MAF=sapply(strsplit(disease_file$INFO,";",fixed=TRUE),"[[", 6)
...
Calculate_IBD=function(data_file,DCV,geneticMap_DIR,gen_allele_mismatch_rate=0.01,MA_cutoff=-0.4,meiosis=1)
{
  ...
  if(!is.data.frame(data_file)){stop("data_file is not a data frame")}
  if(!dir.exists(geneticMap_DIR)){stop("geneticMap_DIR does not exist")}
  if(gen_allele_mismatch_rate>0.05){stop("gen_allele_mismatch_rate is too high")}
  if(MA_cutoff!=-0.4){stop("Please set MA_cutoff=-0.4")}
  ...
  for(j in xL:1)
  {
    ...
    if(k>100)
    {
      ...
      {break}      # IBD has ended, break the loop and stop the Markov chain
  ...
  cumulative_IBD=vector(mode = "numeric",length=nrow(data_file)-xR) # vector to save IBD values at each marker
  allele_mismatch=vector(mode = "numeric",length=nrow(data_file)-xR) # vector to save allele mismatches at each marker; 0= Atleast one allele of the test sample matches with the disease haplotype, 1= both alleles do not match with the disease haplotype
  test_haplotype=vector(mode = "character",length=nrow(data_file)-xR) # vector to save the haplotype the model is on
  IBD_difference=vector(mode = "numeric",length=nrow(data_file)-xR) # vector to save the IBD difference between adjacent markers
  distance_from_the_DCV_cM=vector(mode = "numeric",length=nrow(data_file)-xR) # vector to save the distance markov chain has traversed in cM from the DCV
  ...
  Max_right_cM=data_file[xR+ which(cumulative_IBD==max(cumulative_IBD))[1] -1,"position_cM"]-DCV_cM # the cM location at which the chain gave the highest IBD to the right
